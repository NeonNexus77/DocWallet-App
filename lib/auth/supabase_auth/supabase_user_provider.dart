...
class DocWalletSupabaseUser extends BaseAuthUser {
  DocWalletSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> docWalletSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DocWalletSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
